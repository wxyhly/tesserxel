import { four, mesh, math, util } from '../../build/tesserxel.js';
...
class RubicMgr {
    ...
    move(blcs, generator) {
        this.todoQueue.unshift(new MovingBlcTask(this, blcs, generator));
    }
    steps = 0;
    update() {
        if (!this.currentTask) {
...
class MovingBlcTask {
    ...
    subGenerator;
    blcs;
    constructor(mgr, blcs, generator) {
        this.mgr = mgr;
            ...
            for (let y = 0; y < order; y++) {
                ...
                for (let z = 0; z < order; z++) {
                    ...
                    for (let w = 0; w < order; w++) {
                        ...
                        let cube = mesh.tetra.applyObj4(mesh.tetra.tesseract(), new math.Obj4(pos, null, new math.Vec4(1 - cellGap, 1 - cellGap, 1 - cellGap, 1 - cellGap)));
                        let delnums = [];
                        if (x !== 0)
                            delnums.push(0, 1, 2, 3, 4);
                        if (x !== order - 1)
                            delnums.push(5, 6, 7, 8, 9);
                        if (z !== 0)
                            delnums.push(15, 16, 17, 18, 19);
                        if (z !== order - 1)
                            delnums.push(10, 11, 12, 13, 14);
                        if (w !== 0)
                            delnums.push(25, 26, 27, 28, 29);
                        if (w !== order - 1)
                            delnums.push(20, 21, 22, 23, 24);
                        if (y !== 0)
                            delnums.push(30, 31, 32, 33, 34);
                        if (y !== order - 1)
                            delnums.push(35, 36, 37, 38, 39);
                        let blc = mesh.tetra.setUVWAsPosition(mesh.tetra.clone(cube));
                        let hollow = mesh.tetra.inverseNormal(mesh.tetra.deleteTetras(mesh.tetra.applyObj4(mesh.tetra.clone(hollow_explode_blc), new math.Obj4(pos //, null, new math.Vec4(1 - hollowGap, 1 - hollowGap, 1 - hollowGap, 1 - hollowGap
                        )), delnums));
                        let m1 = new four.Mesh(new four.Geometry(blc), new four.LambertMaterial(rubicBlcColorNode));
                        let m2 = new four.Mesh(new four.Geometry(hollow), new four.LambertMaterial(rubicHColorNode));
                        m1.initPosition = pos;
                        m2.initPosition = pos;
                        m1.peer = m2;
                        m2.peer = m1;
