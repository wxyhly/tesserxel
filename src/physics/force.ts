        ...
        export namespace force {
            ...
            export class Damping extends Force {
                ...
                constructor(linearFactor: number, angularFactor: number | math.Bivec) {
                    ...
                    this.angularFactor = angularFactor instanceof math.Bivec ? angularFactor : new math.Bivec(
            ...
            export type ElectricCharge = { rigid: Rigid | null, position: math.Vec4, worldPos?: math.Vec4, charge: number };
            export type ElectricDipole = { rigid: Rigid | null, position: math.Vec4, worldPos?: math.Vec4, moment: math.Vec4, worldMoment?: math.Vec4 };
            export type MagneticDipole = { rigid: Rigid | null, position: math.Vec4, worldPos?: math.Vec4, moment: math.Bivec, worldMoment?: math.Bivec };
            export type CurrentElement = { rigid: Rigid | null, position: math.Vec4, worldPos?: math.Vec4, current: math.Vec4 };
            export type CurrentCircuit = { rigid: Rigid | null, position: math.Vec4, worldPos?: math.Vec4, current: math.Vec4, radius: number };
            export class MaxWell extends Force {
                ...
                constantElectricField = new math.Vec4;
                /** magnetic field direction is defined by positive charge's velocity wedge it's lorentz force */
                constantMagneticField = new math.Bivec;
                private _vecE = new math.Vec4;
                private _vecdE = new math.Mat4().set();
                private _vecB = new math.Bivec;
                private _vecdB = new math.Matrix(4, 6);
                private _vecP = new math.Vec4;
                ...
                private getEAt(p: math.Vec4, dE: boolean, ignore: Rigid | math.Vec4) {
                    ...
                    for (let s of this.electricCharge) {
                        ...
                        this.addEOfElectricCharge(electricField, dE ? this._vecdE : null, p, s);
                    }
                    for (let s of this.electricDipole) {
                        if (ignore === s.position || ignore === s?.rigid) continue;
                        this.addEOfElectricDipole(electricField, dE ? this._vecdE : null, p, s);
                    }
                }
                private getBAt(p: math.Vec4, dB: boolean, ignore: Rigid | math.Vec4) {
                    let magneticField = this._vecB.copy(this.constantMagneticField);
                    this._vecdB.elem.fill(0);
                    for (let s of this.magneticDipole) {
                        if (ignore === s.position || ignore === s?.rigid) continue;
                        this.addBOfMagneticDipole(magneticField, dB ? this._vecdB : null, p, s);
                    }
                    return magneticField;
                }
                apply(time: number): void {
                    for (let q of this.electricCharge) {
                        q.worldPos ??= new math.Vec4;
                        if (q.rigid)
                    ...
                    for (let q of this.electricCharge) {
                        ...
                        this.getBAt(q.worldPos, false, null);
                ...
                private addEOfElectricCharge(vecE: math.Vec4, dE: math.Mat4, p: math.Vec4, s: ElectricCharge) {

                    let r = math.vec4Pool.pop().subset(p, s.worldPos);
                    let r2 = 1 / r.normsqr();
                ...
                private addBOfMagneticDipole(vecB: math.Bivec, dB: math.Matrix, pos: math.Vec4, s: MagneticDipole) {
                    let k = math.vec4Pool.pop().subset(pos, s.worldPos);
                    ...
                    dB.adds(new math.Matrix(4, 6).set(
                ...
                private addEOfElectricDipole(vecE: math.Vec4, dE: math.Mat4, pos: math.Vec4, s: ElectricDipole) {
                    let r = math.vec4Pool.pop().subset(pos, s.worldPos);
                    let p = s.worldMoment;
                    // u =  r.s / (r.r)^2
