import { ControllerRegistry, ControllerState, KeyState } from "./ctrl.js";
import { KeyBindingUI, shortcut2text } from "./keybind.js";
...
export class SettingGUI {
    ...
    private mainBtnCount = 0;
    private ctrlReg: ControllerRegistry;
    lang?: "zh" | "en";
    isOpen() {
    ...
    private createBtn(desc: BtnDesc) {
        const btn = document.createElement("button");
        btn.className = "setting-gui";
        btn.innerHTML = "&nbsp;";
        if (desc.title?.zh) btn.setAttribute("data-title_zh", desc.title.zh);
        if (desc.title?.en) btn.setAttribute("data-title_en", desc.title.en);
        if (desc.shortcut) btn.setAttribute("data-shortcut", desc.shortcut);
        ...
        const kup = () => { if (keys.get(kname) & 2) keys.set(kname, KeyState.UP); };
        btn.addEventListener('mouseup', kup); btn.addEventListener('mouseout', kup);
        this.refreshBtn(btn);
        return btn;
    }
    private setBtnPos(btn: HTMLButtonElement, lv1: number, lv2: number) {
    ...
    addLvl2Panel(lvl1btn: HTMLButtonElement) {
    ...
    addLvl2Button(desc: BtnDesc, lvl2panel: HTMLDivElement) {
        const btn = this.createBtn(desc);
    ...
    addLvl3Drop(lvl2btn: HTMLButtonElement, width: number, offset: number = 0) {
        const div = document.createElement("div");
    ...
    private refreshBtn(btn: HTMLButtonElement) {
        ...
        if (keybind[0] === ".") keybind = keybind.slice(1);
        let keyText: string;
    ...
    refresh() {
        ...
        // this.lang = (params.get("lang") ?? (navigator.languages.join(",").includes("zh") ? "zh" : "en")) as 'zh' | 'en';
        // this.keybindingMgr.lang = this.lang;
        this.dom.querySelectorAll("button.setting-gui").forEach(btn => this.refreshBtn(btn as HTMLButtonElement));
    }
...
`;
        ...
        for (const a of attr) {
            this.dom.querySelectorAll(`*[data-pos_${a}]`).forEach(
                (e: HTMLButtonElement) => e.style[a] = this.iconSize * Number(e.getAttribute("data-pos_" + a)) + "px"
            );
        }
    }
    increaseBtnSize() {
        if (this.iconSize > 128) return;
        this.iconSize *= 1.25;
