...
export const SVG_HEADER = `<svg xmlns='http://www.w3.org/2000/svg' viewBox='`;
export const SVG_LINE = `style="fill:none;stroke:#FFF;stroke-width:0.25;"`;
...
export class ControllerRegistry {
    ...
    constructor(dom: HTMLElement, ctrls: Array<IController>, config?: ControllerConfig) {
        ...
        this.states.isActionHold = (action, path) => (this.states.isKeyHold(
            keyMap[path + "." + action]
        ) || this.states.currentKeys.get(path + "." + action) === KeyState.DOWN);

        this.states.isActionActive = (action, path) => {
            const str = path + "." + action;
            // direct triggered by event name, not keys
            if (this.states.currentKeys.get(str) === KeyState.DOWN) return true;
            if (this.states.currentKeys.get("." + str)) return true;
        ...
        this.states.isKeyHold = (code) => {
            if (this.gui.isOpen() || !code) return false;
            if (code.includes("|")) {
        ...
        this.evMouseDown = (ev) => {
            ...
            if (this.gui.isOpen()) return false;
    ...
    add(ctrl: IController) {
        ...
        if (ctrl.registGui) ctrl.registGui(this.gui);
    }
...
export class FreeFlyController implements IController {
    ...
    update(state: ControllerState) {
        ...
        const on = (k: string) => state.isActionActive(k, "freefly");
        let delta: number;
        let dampFactor = Math.exp(-this.damp * Math.min(200.0, state.mspf));
        if (enabled) {
...
export class KeepUpController implements IController {
    ...
    update(state: ControllerState) {
        ...
        const on = (k: string) => state.isActionActive(k, "keepup");
        let delta: number;
        let dampFactor = Math.exp(-this.damp * Math.min(200.0, state.mspf));
        if (enabled) {
...
export class RetinaController implements IController {
    ...
    private guiButtons: Record<string, HTMLButtonElement> = {};
    ...
    registGui(gui: SettingGUI) {
        ...
        const _this = this;
        const crosspplBtnFn = function (i: number) {
        ...
        switch (gui.storageMgr.data.preference['stereo']) {
            case "parallel": this.guiButtons["paralleleye"].dispatchEvent(new MouseEvent("mousedown")); break;
            case "off": toggle3DBtn.dispatchEvent(new MouseEvent("mousedown")); break;
        }
    ...
    toggleRetinaAlpha(idx: number) {
        ...
        const guiRefresh = (i: number) => {
            const btn = document.querySelectorAll("span.btn-yellow div button")[i] as HTMLButtonElement;
    ...
    update(state: ControllerState): void {
        ...
        const on = (k: string) => state.isActionActive(k, "retina");
        let delta: number;
        ...
        const refreshGUI = () => {
            ...
            state.storage.save();
            if (!this.guiButtons["crosseye"]) { console.warn("Retina Controller is not registered, cannot update GUI."); return; }
    ...
    toggleSectionConfig(index: string) {
        ...
        const btn = Array.from(document.querySelectorAll("span.btn-red div button")).find(e => e.getAttribute("data-name") === "retina.sectionConfigs." + index) as HTMLButtonElement;
