...
type KeyBindingConfig = {
    ...
    // flatten for controler
    keyIndex: Record<string, string>,
}
let injectedStyle = false;

export class KeyBindingUI extends BaseWindow {
...
.keybind-btn.waiting{
    ...
    private updateActionKey(path: string, action: KeyBindingAction = this.findAction(path), value: string = action.key) {
        ...
        if (k !== this.data.keyIndex[path]) { this.changed = true; }
    }
}
...
export function shortcut2text(lang: "zh" | "en", s: string) {
    if (!s) return "";
    const tr1 = tr[lang];
