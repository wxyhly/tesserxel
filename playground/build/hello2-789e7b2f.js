...
const hello104 = {
    html: defaultHTML,
    js: {
        zh: hello102.js.zh.replace(/默认的四维场景背景[\s\S]+setBackgroundColor.+a: 1\.0 \}\);/, "这里删掉了背景颜色设置，恢复至默认的黑色").replace("定义一个超立方体的几何数据", "定义一个双圆环的几何数据").replace("TesseractGeometry", "TigerGeometry").replaceAll("cubeGeometry", "tigerGeometry").replace("BasicMaterial表示是纯色材质", "PhongMaterial表示光滑材质，有颜色、高光度、高光颜色三个参数，后面两个参数都是可选的，默认值分别为20和白色").replace(".BasicMaterial({ r: 1.0, g: 0.0, b: 0.0, a: 1.0 });", `.PhongMaterial([1, 0, 0], 10);
// 点光源只有光源强度这个参数
const pLight = new FOUR.PointLight([0.9, 0.8, 0.8]);
// 其位置默认位于原点，可通过所有修改其位置的方法来改变位置
pLight.position.w = 2;
// 别忘了把灯光加入场景
app.scene.add(pLight);`), en: hello102.js.en.replace(/The default background color[\s\S]+setBackgroundColor.+a: 1\.0 \}\);/, "Here we removed the background color setting, got default black color").replace("Define the geometry data of a tesseract (hypercube)", "Define the geometry data of a tiger (duotorus)").replace("TesseractGeometry", "TigerGeometry").replaceAll("cubeGeometry", "tigerGeometry").replace("BasicMaterial means a pure color material", `PhongMaterial represents a smooth material, 
