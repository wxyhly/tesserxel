...
// [0.8,1,1,0.2] 等同于 { r: 0.8, g: 1, b: 1, a: 0.1 }
...
class DiceCtrl {
    ...
    update(state) {
        ...
        if (this.target) {
            // 生成从this.dice.rotation出发，到target但只旋转了1/10路程的旋转
...
const mat102 = {
    ...
    js: {
        zh: diceJs, en: diceJs.replace("背景设为天蓝色，为突出骰子，因此降低不透明度", "Set the background to sky blue, and lower the opacity to highlight the dice")
            .replace("[0.8,1,1,0.2] 等同于 { r: 0.8, g: 1, b: 1, a: 0.1 }", "[0.8,1,1,0.2] is equivalent to { r: 0.8, g: 1, b: 1, a: 0.1 }")
            ...
            .replace("否则从当前朝向出发去做lookAt，允许自转自由度", "Otherwise, perform lookAt starting from the current orientation, allowing rotational freedom")
            ...
            .replace("然后在骰子现有的rotation之上，再执行旋转r，得到骰子的最终朝向", "Then apply r on top of the dice’s current rotation to get the final orientation")
            .replace("生成从this.dice.rotation出发，到target但只旋转了1/10路程的旋转", "Generate a rotation starting from this.dice.rotation toward target, but only covering 1/10 of the path")
