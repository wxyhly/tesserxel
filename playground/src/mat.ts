...
const mat101 = {
    ...
    js: {
        zh: mat101_common, en: mat101_common
            .replace("为了能更好控制相机，我们创建App时设置了一个enablePointerLock选项，用于隐藏并锁定鼠标",
                "To better control the camera, we set the enablePointerLock option when creating the App to hide and lock the mouse")
            .replace("超立方体几何数据", "Tesseract geometry data")
            .replace("超球几何数据", "Glome geometry data")
            .replace("地面几何数据，边长为10.0的三维立方体地面",
                "3D cube floor with side length 10.0")
            .replace("创建一个材质颜色变量", "Create a material color variable")
            .replace("材质1为有高光的白色", "Material 1: glossy white")
            .replace("材质2为有高光的绑定了刚才的颜色变量的可变颜色",
                "Material 2: glossy material with the color variable")
            .replace("使用两种材质分别创建两个立方体，并分别设置其位置。",
                "Create two cubes with the two materials and set their positions separately")
            .replace("地板材质", "Floor material")
            .replace("其漫反射颜色使用棋盘格贴图",
                "Its diffuse color uses a checker texture")
            .replace("前两个参数为白色与黑色，第三个参数为贴图坐标",
                "The first two parameters are white and black; the third parameter is texture coordinates")
            .replace("输入默认的立方体的贴图坐标，通过Vec4TransformNode节点进行坐标变换",
                "Input the default cube texture coordinates, and transform them via a Vec4TransformNode node")
            .replace("坐标变换通过math.Obj4对象表示。前两个参数对应平移和旋转，这里都没有",
                "Transformation is represented by a math.Obj4 object. The first two parameters correspond to translation and rotation (none here)")
            .replace("第三个参数为缩放，我们把所有方向均匀缩放10倍以显示更多的棋盘格子",
                "The third parameter is scaling; we scale all directions by 10× to display more checker squares")
            .replace("超球材质为光泽的蓝色",
                "The glome material is glossy blue")
            .replace("故意在y轴z轴上都跟那些超立方体都错开，体现四维的空间感",
                "Intentionally offset along the y- and z-axes from the tesseracts to get the 4D spatial sense")
            .replace("加入所有物体", "Add all objects")
            .replace("加入各类灯光", "Add various lights")
            .replace("spotLight是聚光灯，三个参数分别是RGB颜色强度、锥角与边缘硬度",
                "spotLight's three parameters are RGB intensity, cone angle, and (edge hardness)")
            .replace("下面的物体都要设置动画效果", "The following objects need to be animated")
            .replace("告诉Tesserxel要每帧更新它们的坐标",
                "Tell Tesserxel to update their coordinates each frame")
            .replace("设置相机位置", "Set the camera position")
            .replace("添加一个“保持竖直”模式的相机控制器",
                "Add a camera controller in 'keep-up' mode")
            .replace("生成动画的随机种子", "Generate a random seed for animation")
            .replace("app.run 中的参数是个回调函数，它将在每帧渲染时执行。我们在这里实现动画效果",
                "The parameter of app.run is a callback executed each frame during rendering. We implement animation here")
            .replace("将带参数t的向量单位化后，复制给聚光灯的方向向量",
                "Normalize the vector with parameter t and assign it to the spotlight's direction vector")
            .replace("通过set函数把位置坐标设为三角函数后，再使用mulfs函数修改其值，乘上振幅系数3。",
                "Set the position using trigonometric functions with set(), then modify it with mulfs() multiplying by amplitude factor 3")
            .replace("也可以先赋值，再通过norms函数将其修改为单位向量。",
                "You can also assign values first then normalize it with the norms() function")
            .replace("给material2材质中的颜色变量动态赋值，实现右边的超立方体的变色效果",
                "Dynamically assign values to the color variable in material2 to achieve the color-changing effect of the right tesseract")
