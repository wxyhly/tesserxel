...
class StructBuilder {
    ...
    forces;
    bondObj;
    constructor(rank, pair, atomGeom, bondGeom, bondMat) {
    ...
    calcEnergy() {
        ...
        for (let a_id = 0; a_id < this.molObj.child.length; a_id++) {
            const a = this.molObj.child[a_id];
            for (let b_id = 0; b_id < this.molObj.child.length; b_id++) {
                if (a_id === b_id)
                    continue;
                const b = this.molObj.child[b_id];
    ...
    update() {
        ...
        for (let idx = 0; idx < this.molObj.child.length; idx++) {
            const a = this.molObj.child[idx];
            const b = this.bondObj.child[idx];
            const v1 = a.position;
                ...
                for (let a2 = -10; a2 <= 10; a2++) {
                    for (let a3 = -10; a3 <= 10; a3++) {
                        ...
                        for (let j = 0; j < 5; j++) {
                            ...
                            b.scale.set(0.005, 0.005, 0.005, v1.norm());
                            if ((pos.normsqr() < 5))
                                b.scale.set(0.01, 0.01, 0.01, v1.norm());
                            this.bondObj.add(b);
                        }
                    }
                }
...
export var hyperdiamond;
(function (hyperdiamond) {
    async function load() {
