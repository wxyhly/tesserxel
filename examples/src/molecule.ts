...
class StructBuilder {
    debug: tesserxel.math.Vec4[] = [];
    debugMesh: tesserxel.four.Object[] = [];
    graph: Graph;
    ...
    calcForce() {
        ...
        // pi bond constrain
        const accumAtoms = new Array<AtomNode>;
        for (let b of this.graph.bonds) {
            // ignore X-Y  and -X#Y  X#Y  X#Y-
            if (b.type >> 4 <= 1 || b.start.bonds.length <= 1 || b.stop.bonds.length <= 1) continue;
            // planar sp2 - sp2
            if (b.start.piOrbitals === 2 && b.stop.piOrbitals === 2) {
                ...
                // len 1~3
                if (an.length === 0 || an.length > 3 || bn.length === 0 || bn.length > 3) continue;
                if (an.length === 1 && bn.length === 1) continue;
                const k = tesserxel.math.vec4Pool.pop();
                const k2 = tesserxel.math.vec4Pool.pop();
                const k3 = tesserxel.math.vec4Pool.pop();
                const kb = tesserxel.math.vec4Pool.pop().subset(b.start.position, b.stop.position);
                let step: number, torque: tesserxel.math.Bivec;
                ...
                if (b.type >> 4 === 2 && b.start.piOrbitals !== b.stop.piOrbitals) accumAtoms.push(b.start.piOrbitals === 1 ? b.start : b.stop);
                if (b.type >> 4 === 3 && b.start.piOrbitals !== b.stop.piOrbitals) accumAtoms.push(b.start.piOrbitals === 2 ? b.start : b.stop);
            }
        }
        let otherSide: AtomNode[] = [];
        // multiple pi bonds
        for (let start of accumAtoms) {
            if (otherSide.includes(start)) continue;
            ...
            let normal: tesserxel.math.Vec4 | tesserxel.math.Bivec = b.type >> 4 === 2 ?
            ...
            let normal2: tesserxel.math.Vec4 | tesserxel.math.Bivec = b.type >> 4 === 2 ?
                ...
                ;
            this.debug[1] = normal.clone() as tesserxel.math.Vec4;
            this.debug[2] = normal2.clone() as tesserxel.math.Vec4;


            let step: number; let torque: tesserxel.math.Bivec;
            if (normal instanceof tesserxel.math.Vec4 && normal2 instanceof tesserxel.math.Vec4) {
                step = -100;
                if (normal.dot(normal2) < 0) normal2.negs();
                ...
                step = 0; torque = new tesserxel.math.Bivec();
            }
            for (const pa of an) {
    ...
    buildAndAddToScene(scene: tesserxel.four.Scene) {
        ...
        for (let b of graph.bonds) {
            if (b.type === 8 || b.start.name === "Elc" || b.stop.name === "Elc") continue;
        ...
        for (let i = 0; i < 50; i++)this.iterate(0.0001);
    ...
    changeStickMode() {
        ...
        for (const a of this.graph.atoms) {
            if (!a.obj) continue;
            const r = this.stickMode ? a.radius * 0.35 : a.radius;
            a.obj.scale ??= new tesserxel.math.Vec4; a.obj.scale.set(r, r, r, r);
        }
    }
}
