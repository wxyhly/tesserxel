...
class ShapesApp {
    ...
    wireFrameScene: tesserxel.four.WireFrameScene;
    async init(fragmentShaderCode: string, mesh: tesserxel.mesh.TetraMesh, wireFrameScene?: tesserxel.four.WireFrameScene) {
...
export namespace tesseract_ortho {
    export async function load() {
        ...
        app.trackBallController.object.rotation.setFromLookAt(
            tesserxel.math.Vec4.x,
            new tesserxel.math.Vec4(1, Math.SQRT1_2, 0, -Math.SQRT1_2).norms()
        ).mulsl(tesserxel.math.Rotor.lookAt(
            tesserxel.math.Vec4.y.rotate(app.trackBallController.object.rotation),
            new tesserxel.math.Vec4(0, Math.SQRT1_2, 1, Math.SQRT1_2).norms()
        )).conjs();
...
export namespace suzanne3d {
    export async function load() {
        ...
        let mesh = new tesserxel.mesh.FaceIndexMesh(meshFile.parse()).applyObj4(new tesserxel.math.Obj4(
            new tesserxel.math.Vec4(-s)
        )).toNonIndexMesh();
...
async function makeProduct(src1: string, src2: string) {
