...
async function loadSTLinks(pointsOnS2: tesserxel.math.Vec3[], factor: number, staticMode?: boolean) {
    ...
    function run() {
        ...
        renderer.render(scene, camera);
        // simulating physics
        if (!staticMode) engine.update(world, Math.min(1 / 15, controllerRegistry.states.mspf / 1000) / pointsOnS2.length);
        window.requestAnimationFrame(run);
    }
    run();
}
...
export namespace m_dipole_dual2 {
    export async function load() {
        await loadMaxwell(async (world, maxwell, scene, renderer) => {
            const phyMatCharge = new phy.Material(1, 0.5);
            const chargeNum = 8;
...
class gravityApp {
    async load(initVelocity: number, G: tesserxel.physics.Gravity) {
        const engine = new phy.Engine({ substep: 100 });
        const world = new phy.World();
        const scene = new FOUR.Scene();
        let camera = new FOUR.PerspectiveCamera();
