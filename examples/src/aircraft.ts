...
const phyHelper: [tesserxel.physics.Rigid, tesserxel.four.Object][] = [];
export namespace aircraft {
    ...
    export async function load() {
        ...
        const springts: tesserxel.physics.TorqueSpring[] = [];
        const damping = new PHY.Damping(0, 0.001);
        function makeGearP(mass: number, x: number, z: number, w: number) {
        ...
        function run() {
            ...
            if (aircraftCtrl.camLock) {
                if (aircraftCtrl.camPos !== -1) freeCamCtrl.updateObj();
            ...
            for (const st of springts) {
                ...
                if (st.b) dstB.rotates(st.b.rotation);
    ...
    async function genAircraftMesh() {
        ...
        const aircraftBodyMat = new FOUR.PhongMaterial(new FOUR.WgslTexture(
            `fn aircraftbody_texture(uvw:vec4<f32>)->vec4<f32>{
        ...
        const aircraftBody = new FOUR.Mesh(new FOUR.Geometry(
            tesserxel.mesh.tetra.rotatoid(new tesserxel.math.Bivec(0, 0, -1), aircraftBodyMesh3d, 36).inverseNormal().setUVWAsPosition()
        ), aircraftBodyMat);
        ...
        function makeGearWheelMesh() {
            ...
            gearWheel.add(
                new FOUR.Mesh(gearwheelTireGeom, gearwheelTireMat).rotatesb(Bivec.xy.add(Bivec.zw).mulfs(Math.PI / 2)),
                new FOUR.Mesh(gearwheelAxleGeom, steelMat).rotatesb(Bivec.yw.mulf(Math.PI / 2)),
            )
            return gearWheel;
        }
        const dist = 0.3;
        ...
        aircraftWingCW.data[0].push(
            ...wingDataIdx.map((i, id) => new Vec4(wingData[i].x, wingData[i].y, -wingData[i].z, wingWData[wingDataIdx[id | 4]]))
        );
        ...
        const aircraftEngineShellGeom = new FOUR.Geometry(
            tesserxel.mesh.tetra.rotatoid(Bivec.yw, aircraftEngineMesh3d, 24)
        );
        ...
        aircraftHTailCW.data[0].push(
            ...HTailDataIdx.map((i, id) => new Vec4(HTailData[i].x, 0.735556 + HTailData[i].z, -HTailData[i].y, 0.735556 + HTailWData[HTailDataIdx[id | 1]]))
        );
    ...
    function genAircraftTrack() {
        const track = new FOUR.Mesh(new tesserxel.four.CubeGeometry(new tesserxel.math.Vec3(13, 300, 13)), new FOUR.LambertMaterial(new FOUR.WgslTexture(
            `fn track(uvw:vec4<f32>)->vec4<f32>{
                let auvw = abs(uvw);
...
class AircraftCtrl implements tesserxel.util.ctrl.IController {
    ...
    updateHud() {
        ...
        _v.copy(Vec4.z).rotates(this.rigid.rotation); _v.y = 0; _v.norms();
...
class AircraftMecanism extends tesserxel.physics.Force {
    engine = 0;
    rigid: tesserxel.physics.Rigid;
    controls: typeof AircraftCtrl.prototype.controls;
    constructor(ctrl: AircraftCtrl) {
        super();
    ...
    private applyForce(force: tesserxel.math.Vec4, position: tesserxel.math.Vec4) {
        this.localForce.adds(force);
        this.localTorque.adds(position.wedge(force));
    }
    private applyWorldForce(force: tesserxel.math.Vec4, position: tesserxel.math.Vec4) {
    ...
    apply(time: number): void {
        ...
        this.applyForce(new Vec4(
            -windVelocity.x * 0.3,
            -windVelocity.y * 0.5 - windVelocity.w * (0.4 + this.controls.aileron1 * 3) * 0.2,
            -windVelocity.z * 0.3, -windVelocity.w * (0.05 + Math.abs(this.controls.aileron1) * (Math.sign(this.controls.aileron1) * 0.05 + 0.1))
        ), new Vec4(2, ycenter, 0, wingPosW));
        this.applyForce(new Vec4(
            -windVelocity.x * 0.3,
            -windVelocity.y * 0.5 - windVelocity.w * (0.4 + this.controls.aileron2 * 3) * 0.2,
            -windVelocity.z * 0.3, -windVelocity.w * (0.05 + Math.abs(this.controls.aileron2) * (Math.sign(this.controls.aileron2) * 0.05 + 0.1))
        ), new Vec4(-1, ycenter, Math.sqrt(3), wingPosW));
        this.applyForce(new Vec4(
            -windVelocity.x * 0.3,
            -windVelocity.y * 0.5 - windVelocity.w * (0.4 + this.controls.aileron3 * 3) * 0.2,
            -windVelocity.z * 0.3, -windVelocity.w * (0.05 + Math.abs(this.controls.aileron3) * (Math.sign(this.controls.aileron3) * 0.05 + 0.1))
        ), new Vec4(-1, ycenter, -Math.sqrt(3), wingPosW));

        this.applyForce(new Vec4(
            -windVelocity.x * 0.3 - windVelocity.w * 0.2 * this.controls.rudderX,
            -windVelocity.y * 0.5 - windVelocity.w * 0.09,
            -windVelocity.z * 0.3 - windVelocity.w * 0.2 * this.controls.rudderY,
            -windVelocity.w * 0.05
        ).mulfs(0.4), new Vec4(0, ycenter, 0, tailHPosW));

        //flap
        this.applyForce(new Vec4(
            0,
            0 - windVelocity.w * (0.2 + 0.2 * this.controls.flap),
            0, -windVelocity.w * 0.5 * this.controls.flap
        ).mulfs(0.4), new Vec4(0, ycenter, 0, wingPosW));
        ...
        if (wv > 0.05) {
            ...
            const lv0 = new Vec4(); this.rigid.getlinearVelocity(lv0, gear0); lv0.y = 0;
            const lv1 = new Vec4(); this.rigid.getlinearVelocity(lv1, gear1); lv1.y = 0;
            const lv2 = new Vec4(); this.rigid.getlinearVelocity(lv2, gear2); lv2.y = 0;
            const lv3 = new Vec4(); this.rigid.getlinearVelocity(lv3, gear3); lv3.y = 0;
            const rx = Vec4.x.rotate(this.rigid.rotation); rx.y = 0; rx.norms();
            const rz = Vec4.z.rotate(this.rigid.rotation); rz.y = 0; rz.norms();
            const rw = Vec4.w.rotate(this.rigid.rotation); rw.y = 0; rw.norms();
            const lv0mag = lv0.norm();
            const lv1mag = lv1.norm();
            const lv2mag = lv2.norm();
            const lv3mag = lv3.norm();
            if (gear0.y < 0.8) { const f = new Vec4(12 * lv0.dot(rx) / lv0mag, 0, 12 * lv0.dot(rz) / lv0mag, 0).negs().rotates(this.rigid.rotation); f.y = 0; this.applyWorldForce(f, gear0); }
            if (gear1.y < 0.8) { const f = new Vec4(12 * lv1.dot(rx) / lv1mag, 0, 12 * lv1.dot(rz) / lv1mag, this.controls.brake1 * 12 * lv1.dot(rw) / lv1mag).negs().rotates(this.rigid.rotation); f.y = 0; this.applyWorldForce(f, gear1); }
            if (gear2.y < 0.8) { const f = new Vec4(12 * lv2.dot(rx) / lv2mag, 0, 12 * lv2.dot(rz) / lv2mag, this.controls.brake2 * 12 * lv2.dot(rw) / lv2mag).negs().rotates(this.rigid.rotation); f.y = 0; this.applyWorldForce(f, gear2); }
            if (gear3.y < 0.8) { const f = new Vec4(12 * lv3.dot(rx) / lv3mag, 0, 12 * lv3.dot(rz) / lv3mag, this.controls.brake3 * 12 * lv3.dot(rw) / lv3mag).negs().rotates(this.rigid.rotation); f.y = 0; this.applyWorldForce(f, gear3); }
        }
